...
const outputDirectoryPath = 'D:/WorkSpace/e_drive/202306/Response/ran_metrics';
...
async function runIterations() {
  ...
  for(let i = 0; i < cnt; i++){
    ...
    if (editingcode_res.length !== 3) {
      ...
      // CSV にデータを追加
      csvContent += `${i + 1},${selectedFile},${editingcode_res[1]},${matchingFiles},${cnt - i},"${recommendedFilePathList.join(';')}"\n`;
    }
    
  }

  // CSV ファイルに書き込み
  //fs.writeFileSync(`C:/users/unico/desktop/tmp/tmp_${formattedDate}.csv`, csvContent, 'utf-8');
  ...
  if (recommendedFilePathIndex === -1) {
    ...
    // 結果を新しいCSVファイルに書き込み
    //fs.writeFileSync(`C:/users/unico/desktop/tmp/tmp_${formattedDate}.csv`, resultCsvContent, 'utf-8');
  ...
  file2_stream
    ...
    .on('data', (row) => {
      try {
        ...
        if (recomTar !== 0 && totalFiles !== 0 && matchingFiles !== 0) {
          ...
          if (!evaluationMetrics[endp] || recall > evaluationMetrics[endp].recall) {
            // 新しい最大Recallの場合、他の指標も計算して更新
    ...
    .on('end', () => {
      ...
      // 最大Recallを持つエンドポイントの評価指標
      const maxRecallEndp = Object.keys(evaluationMetrics).reduce((a, b) => evaluationMetrics[a].recall > evaluationMetrics[b].recall ? a : b);
      console.log(`最大Recallを持つエンドポイント(${maxRecallEndp}):`, evaluationMetrics[maxRecallEndp]);

      // 最小Recallを持つエンドポイントの評価指標
      const minRecallEndp = Object.keys(evaluationMetrics).reduce((a, b) => evaluationMetrics[a].recall < evaluationMetrics[b].recall ? a : b);
      console.log(`最小Recallを持つエンドポイント(${minRecallEndp}):`, evaluationMetrics[minRecallEndp]);

      // ファイルにデータを書き込み
      // const filePath = `E:/Files/workspace/202306/Response/ran_metrics/temp/temp_${formattedDate}.json`;
