...
function find_url(ast) {
  var cnt = ast.body.length - 1;

  while (cnt >= 0) {
    var tmp = find_loop(ast.body[cnt]);
    if (tmp !== null) {
      return tmp;
    }
    cnt--;
...
function find_loop(ast) {
  ...
  if (ast.type === 'ReturnStatement') { // http探す
...
// ASTから特定の識別子を抽出し、置換する関数
function replaceIdentifierWithPlaceholder(ast, identifierName, placeholder) {
  if (ast == null) {
    return null;
  }

  if (ast.type === 'Identifier' && ast.name === identifierName) {
    return { type: 'Identifier', name: placeholder };
  }

  if (Array.isArray(ast)) {
    return ast.map(item => replaceIdentifierWithPlaceholder(item, identifierName, placeholder));
  }

  if (typeof ast === 'object') {
    const newObj = {};
    for (const key in ast) {
      newObj[key] = replaceIdentifierWithPlaceholder(ast[key], identifierName, placeholder);
    }
    return newObj;
  }
...
// エンドポイントの比較を行う関数
function compareEndpoints(ast, baseJson) {
  const result = find_url(ast);

  if (result && result.type === 'FunctionDeclaration' && result.body && result.body.type === 'BlockStatement' && Array.isArray(result.body.body)) {
    const url = result.body.body[0];
    if (typeof url === 'string') {
      const urlParts = url.split('+{applianceid}+');

      if (urlParts.length === 2) {
        const modifiedUrl = urlParts[0] + '{applianceid}' + urlParts[1];
        const matchingEndpoints = baseJson.nature_apis.filter(api => modifiedUrl.includes(api.endpoint));



        if (matchingEndpoints.length > 0) {
          console.log('Match found for endpoint:', modifiedUrl);
          matchingEndpoints.forEach(api => {
            console.log(api.endpoint);
          });
        } else {
          console.log('No match found for endpoint:', modifiedUrl);
        }
      }
    }
  }
...
function comp_Api(ast) {
  ...
  const modifiedAst = replaceIdentifierWithPlaceholder(ast, 'light_id', '{applianceid}');
  compareEndpoints(modifiedAst, baseJson);
}