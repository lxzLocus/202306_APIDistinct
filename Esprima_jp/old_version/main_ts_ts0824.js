...
function comp_api(ast) {
  ...
  if(url_logic[0] == true){
    ...
    if(opt_logic[0] == true){
      ...
      /*payload有無*/
      var payload_logic = payload_initial(ast)
      if(payload_logic[2] === 1 || payload_logic[2] === 2){
        console.log('変数', payload_logic[0], 'が見つかりませんでした\n')
...
function param_initial(ast, json, json_num){
  ...
  let json_url = json.servers[0].url + json.nature_apis.api[json_num].name;
  ...
  if(json.nature_apis.api[json_num].parameters == null){
    ...
    json_parameter = json.nature_apis.api[json_num].parameters.name;
  ...
  if (json.nature_apis.api[json_num].requestBody == null){
    ...
    json_payload = json_request_schema.replace('/', '')
    json_payload = json_request_schema.replace('/', '')
    json_payload = json_request_schema.substr(json_request_schema.indexOf('/') + 1)
...
function find_param(ast, pay_flag, callee_flag, opt_flag, param0, param1) {
  if (ast === null){
    return null;
  }
  ...
  if (ast.type === 'BlockStatement') { // 複数行の中身
    let val = find_param(ast, pay_flag, callee_flag, opt_flag, param0, param1);

    return val;
  }

  if (ast.type === 'ExpressionStatement') {
    return find_param(ast.expression, pay_flag, callee_flag, opt_flag, param0, param1);
  }

  if (ast.type === 'VariableDeclaration') { // 代入式
    if (ast.kind === 'var' || ast.kind === 'const' || ast.kind === 'let') {
      const body = find_param(ast.declarations[0].init, pay_flag, callee_flag, opt_flag, param0, param1);
      ...
      if(ast.declarations[0].id.name === param0 && opt_flag === 1){
        ...
        let tmp = find_param(ast.declarations[0].init, pay_flag, callee_flag, opt_flag, param0, param1);
  ...
  if (ast.type === 'FunctionDeclaration') { // ユーザ関数呼び出し
    const body = find_param(ast.body, pay_flag, callee_flag, opt_flag, param0, param1);
  ...
  if (ast.type === 'ObjectExpression'){
    for (let i = ast.properties.length -1; i >= 0; i--){
      let tmp = find_param(ast.properties[i], pay_flag, callee_flag, opt_flag, param0, param1);
  ...
  if(ast.type === 'Property' && ast.kind === 'init'){
    ...
    /*
    const body = find_param(ast.key, pay_flag, callee_flag, opt_flag, param0, param1);

    if (ast.key.value === param0 && opt_flag === 1){
      //検出
      opt_flag = 3;
      let tmp = find_param(ast.key, pay_flag, callee_flag, opt_flag, param0, param1);
