...
//API取得
const base_json = JSON.parse(fs.readFileSync('E:/Files/workspace/202306/nature_sort.json', 'utf8'));
...
function find_loop(ast) {
  ...
  if (ast.type == 'ExpressionStatement') {
    return simplify(ast.expression,option,keys,payload);
  }

  if (ast.type == 'VariableDeclaration') {//代入式
    let tmp = find_loop(ast.left)
  ...
  if (ast.type == 'BlockStatement') {//複数行の中身
    let tmp = find_url(ast)

    return tmp;
  }

  if (ast.type == 'WhileStatement') {//While文
    let tmp = find_loop(ast.test);
  ...
  if (ast.type == 'ArrayExpression') {//配列
    ...
    return ast.elements.map(element => find_loop(element));
  }

  if (ast.type == 'MemberExpression') {//配列を参照する
    return find_loop(ast.object) + '.' + find_loop(ast.property);
  }

  if (ast.type == 'ObjectExpression') {//ハッシュ作成
    const obj = {};
    ast.properties.forEach(property => {
      obj[find_loop(property.key)] = find_loop(property.value);
    });
  ...
  if (ast.type == 'FunctionDeclaration') {//ユーザ関数呼び出し
    let tmp = find_loop(ast.body)

    return tmp;
  }

  if (ast.type == 'ReturnStatement') {//http探す
    let tmp = find_loop(ast.argument)
...
function comp_Api(ast){
  var trgt_url;
  trgt_url = find_url(ast);

  console.log(trgt_url);

  while(base_json.name != trgt_url){
    console.log('discov')
  }
}
