...
const outputDirectoryPath = 'C:/Users/unico/Desktop/debug_res';
...
// ファイル名を組み立て
const outputFileName = `${formattedDate}.csv`;
...
/*CSV ファイルのヘッダ*/
let csvContent = 'Iteration,SelectedFile,MatchingFiles,TotalFiles,RecommendedFilePath\n';
let editingcode_res

/*ファイルリスト*/
const fileListOutputPath = path.join(outputDirectoryPath, 'fileList.csv');
...
for (let i = 0; i < ttl_files; i++) {
  ...
  if (editingcode_res.length !== 3) {
    ...
    // 推薦コードが true になったファイルパス
    let recommendedFilePath = '';
    filePathList.forEach((filePath) => {
      let rtn;
      rtn = main(filePath);

      if (rtn[0] === true && rtn[4] === true && rtn[1] === editingcode_res[1]) {
        recommendedFilePath = filePath;
        console.log('推薦コード');
      }
    });

    // CSV にデータを追加
    csvContent += `${i + 1},${selectedFile},${matchingFiles},${ttl_files},${recommendedFilePath}\n`;
  }
}
