...
function param_initial(ast, json, json_num){
  ...
  let prm = loop_param_method(ast, info_flag);
  source = prm[0]
  pay_flag = prm[2];
  opt_flag = prm[2];
  ...
  try{
    if(json_method === 'POST'){
      ...
      prm = loop_param_payload(ast, info_flag);
      source = prm[0];
      pay_flag = prm[2];
      opt_flag = prm[2];
...
function find_param_method(ast, pay_flag, callee_flag, opt_flag, info_flag, param0, param1) {
  if (ast === null){
    return null;
  }
  ...
  if (ast.type === 'BlockStatement') { // 複数行の中身
    let val = loop_param_method(ast, info_flag);

    return val;
  }

  if (ast.type === 'ExpressionStatement') {
    return find_param_method(ast.expression, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  }
  ...
  if (ast.type === 'VariableDeclaration') { // 代入式
    if (ast.kind === 'var' || ast.kind === 'const' || ast.kind === 'let') {
      ...
      const body = find_param_method(ast.declarations[0].init, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
      ...
      if(ast.declarations[0].id.name === param0 && opt_flag === 1 ){
        ...
        /*項目探索*/
        let tmp = find_param_method(ast.declarations[0].init, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if (ast.type === 'FunctionDeclaration') { // ユーザ関数呼び出し
    const body = find_param_method(ast.body, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if (ast.type === 'SwitchStatement') {
    ...
    for (let i = ast.cases.length - 1; i >= 0; i--) {
      ...
      var tmp = find_param_method(ast.cases[i], pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if (ast.type === 'ObjectExpression'){
    for (let i = ast.properties.length -1; i >= 0; i--){
      ...
      var tmp = find_param_method(ast.properties[i], pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if(ast.type === 'Property' && ast.kind === 'init'){
    ...
    if (ast.key.value === 'method' && info_flag === 0) {
      ...
      var tmp = find_param_method(ast.value, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
...
function loop_param_method(ast, info_flag){
  /*Initialization*/
  var pay_flag = 0
  var callee_flag = 0
  var opt_flag = 0
  ...
  while(ast.body[i + cnt]){
    ...
    var tmp = find_param_method(line, pay_flag, callee_flag, opt_flag,  info_flag, param[0], param[1]);
...
function find_param_payload(ast, pay_flag, callee_flag, opt_flag, info_flag, param0, param1) {
  if (ast === null){
    return null;
  }
  ...
  if (ast.type === 'BlockStatement') { // 複数行の中身
    let val = loop_param_payload(ast, info_flag);

    return val;
  }

  if (ast.type === 'ExpressionStatement') {
    return find_param_payload(ast.expression, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  }
  ...
  if (ast.type === 'VariableDeclaration') { // 代入式
    if (ast.kind === 'var' || ast.kind === 'const' || ast.kind === 'let') {
      ...
      const body = find_param_payload(ast.declarations[0].init, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
      ...
      if(ast.declarations[0].id.name === param0 && opt_flag === 1 ){
        ...
        /*項目探索*/
        let tmp = find_param_payload(ast.declarations[0].init, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if (ast.type === 'FunctionDeclaration') { // ユーザ関数呼び出し
    const body = find_param_payload(ast.body, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if (ast.type === 'SwitchStatement') {
    ...
    for (let i = ast.cases.length - 1; i >= 0; i--) {
      ...
      var tmp = find_param_payload(ast.cases[i], pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if (ast.type === 'ObjectExpression'){
    for (let i = ast.properties.length -1; i >= 0; i--){
      ...
      var tmp = find_param_payload(ast.properties[i], pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
  ...
  if(ast.type === 'Property' && ast.kind === 'init'){
    ...
    if (ast.key.value === 'method' && info_flag === 0) {
      ...
      var tmp = find_param_payload(ast.value, pay_flag, callee_flag, opt_flag, info_flag, param0, param1);
...
function loop_param_payload(ast, info_flag){
  /*Initialization*/
  var pay_flag = 0
  var callee_flag = 0
  var opt_flag = 0
  ...
  while(ast.body[i + cnt]){
    ...
    var tmp = find_param_payload(line, pay_flag, callee_flag, opt_flag,  info_flag, param[0], param[1]);
...
function payload_initial(ast) {
  ...
  for (let i = ast.body[0].body.body.length - 1; i >= 0; i--){
