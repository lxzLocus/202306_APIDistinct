...
function comp_api(ast) {
  const jsonFilePath = 'E:/Files/workspace/202306/API_json/nature_sort_v2.json';
  ...
  if(url_logic[0] == true){
    ...
    /*requestBodyの中身*/
    var param = param_initial(url_logic[0], url_logic[1], baseJson)
    ...
    if(payload_logic[2] === 1 || payload_logic[2] === 2){
      ...
      console.log(payload_logic[0], payload_logic[1])
      /////////////正しいとき（順当に行ったとき）の処理/////////////
      //var match = match_reqbody(payload_logic[0], payload_logic[1], find_param)
...
function param_initial(url, json_num, json){
  var json_schema = json.nature_apis[json_num].requestBody.content["application/x-www-form-urlencoded"].schema['$ref']

  json_schema = json_schema.replace('/', '')
  json_schema = json_schema.replace('/', '')
  json_schema = json_schema.substr(json_schema.indexOf('/')+ 1)
...
function url_initial(ast, json){
  ...
  if(opt_flag == 1){
    ...
    /*一致URLを探す*/
    while (undefined != json.nature_apis[cnt_w]) {
      var JsonEndpoint = json.nature_apis[cnt_w].endpoint
      ...
      if(cnt_w != 0){
        ...
        if (logic_flag[0] == true) {
          ...
          let server_url_split = url_split(json.nature_apis[0].servers.url)

          /*servers.urlが一致するか*/
          for(let p = 0; p < server_url_split.length; p++){
            if (url_source_split[p] != server_url_split[p]){

              console.log("ベースURL 一部誤り\n")
...
function url_split(url){
  ...
  const beforeLength = beforeBrace.split("/").length;

  for (let i = beforeLength; i < parts.length; i++) {
...
while (undefined != json.nature_apis[cnt_w]) {
  ...
  if (url_source_split[0].includes(url_json_split[0]) && url_json_split[1] === url_source_split[1]) {
    logic_flag = true;
