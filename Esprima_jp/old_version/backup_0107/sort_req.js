...
// ファイル名を組み立て
const outputFileName = `sort_${formattedDate}.csv`;
...
/*ファイルリスト*/
const fileListOutputPath = path.join(outputDirectoryPath, 'sort_fileList.csv');
let fileListContent = 'FileList\n';

// ファイルパスのリストをアルファベット順にソート
filePathList.sort();  // 降順にソート
...
for (let i = 0; i < ttl_files; i++) {
  /*Editing Code (Descending order)*/
  let selectedFile = getNextFilePath(filePathList);  // 降順に次のファイルを取得
...
/*ランダム化*/
function getNextFilePath(filePathList) {
  if (filePathList.length === 0) {
    // リストが空の場合は null を返す
    return null;
  }

  // ファイルパスリストが降順にソートされている前提で、先頭から順に取得
  const nextFilePath = filePathList.shift();
